-- =====================================================
-- WORKING MIGRATION SCRIPT - MATCHES ACTUAL POLICIES SCHEMA
-- =====================================================
...
-- Step 2: Load Insurance Companies (has UNIQUE constraint on name)
...
-- Step 3: Load Policy Types (has UNIQUE constraint on slug)
...
-- Insert commercial types (has UNIQUE constraint on slug)
...
-- Insert personal types (has UNIQUE constraint on slug)
INSERT INTO policy_types (id, display_name, slug, created_at, updated_at)
SELECT 
    gen_random_uuid() as id,
    insurance_type as display_name,
...
-- Step 4: Load Claim Types (has UNIQUE constraint on name)
...
-- Step 5: Load Clients (NO UNIQUE constraint on id_number - removed ON CONFLICT)
...
-- Note: Removed ON CONFLICT (id_number) DO NOTHING - no unique constraint exists

-- Step 6: Load Addresses (NO UNIQUE constraints - removed ON CONFLICT)
...
-- Note: No ON CONFLICT clause needed - no unique constraints

-- Step 7: Load Policies (FIXED - matches actual schema)
...
-- Note: Removed premium/excess checks since those columns don't exist
